...
def _persist_and_route(state: dict, outcome: str):
    ...
    try:
        decision = Decision(
            ...
            remediation_proposals = state.get("proposals") or [],
            critic_verdicts       = state.get("verdicts") or [],
            round_count           = state.get("round_num") or 1,
            consensus_reached     = bool(state.get("consensus_reached")),
            final_action          = state.get("final_action") or "escalate_no_op",
            risk_tier             = state.get("risk_tier") or "high",
